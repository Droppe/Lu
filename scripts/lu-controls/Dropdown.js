...
Dropdown =  Class.create( Abstract,  ( function () {
  ...
  return {
    ...
    initialize: function ( $super, $element, settings ) {
       ...
       * @param {String} classNameQuery - CSS class query to search for (ex. ".lu-item" )
       * @method findLuItems
       * @private
       * @return {Collection} Collection of JQuery DOM link elements
       */
      function findLuItems( $ul, classNameQuery ) {
        var $elements;

        if ( $ul ) {
          // Find the link in a list and find the "lu-item" class and if that's not 
       ...
       * is the innerHTML of the link or the one with class .lu-item.
       * @param {Event} event - Lu event
       * @param {Object} item - JQuery DOM element
      ...
      function selectItemHandler( event, item ) {
        var $elements = findLuItems( $dropDownList, '.lu-item' ),
            $link;
      ...
      function selectedItemHandler( event, item, index ) {
        ...
        itemName = findLuItems( $item, '.lu-item' ).html(); 
      ...
      function handleKeyup( event, item ) {
        var keyCode = event.keyCode,
            $item = $( event.target ),
            list =  $dropDownList.lu( 'getControl', 'List' );
