...
Button = Switch.extend( function( Switch ){
  ...
  function defaultDecorator( Button, settings ){
    ...
    return function( Button ){
      Button.on( on,     command = settings.action,
        decorators = [];
      ...
      switch( command ){
        case 'state':
          decorators.push( commandDecorators[command]( Button, _.defaults( settings, orators.push( defaultDecorator( Button, settings ) );
        ...
        case 'next':
          _.defaults( settings, defaults );
          decorators.push( defaultDecorator( Button, sett, settings ) );
          break;
        case 'load':
          _.defaults( settings, defaults );
          decorators.push( commandDecorators[command]( Button, settings ) );
      ...
      //somthing like the below shoument = this.$element;
      if( $element.is( HAS_A18_ATTRS ) ){
