...
function execute(){
  ...
  QUnit.asyncTest( 'Item Select by Enter Key', function(){
    ...
    DropdownA.one( 'stated', function(){
      ok( $btnA.html() === $li.html(), 'Label updated with list items\'s text.' );
      DropdownA.listInstance.select( 0 );
      ok( DropdownA.listInstance.index() === 0, 'Selected index reset back to 0.');
      start();
    } );
  ...
  QUnit.asyncTest( 'Reset List', function(){
    var event = $.Event( 'keydown' );
    event.keyCode = 40;
  ...
  QUnit.asyncTest( 'Updated Event', function(){
    expect( 2 );
    ...
    DropdownA.one( 'updated', function( evt, instance ){ 
      ok( evt.type === 'lu:updated', 'Triggered updated event.' );
      DropdownA.listInstance.select( 0 );
      start();
    } );
